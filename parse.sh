...
do
    ...
    img_medium=$(jq -r '.illusts['$i'].image_urls.medium' tmp/pixiv.json | sed "s~_master1200~~g")
    img_large=$(jq -r '.illusts['$i'].image_urls.large' tmp/pixiv.json | sed "s~_master1200~~g")
